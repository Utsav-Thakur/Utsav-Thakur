...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&duration=2800&pause=700&color=8B949E&center=true&vCenter=true&width=650&height=40&lines=Data+Scientist+%26+AI+Systems+Builder;Graph+Intelligence+%26+LLM+Platform+Engineer;I+don't+just+study+ML+%E2%80%94+I+ship+it." alt="Typing SVG" />

<br>

[![Profile Views](https://komarev.com/ghpvc/?username=Utsav-Thakur&style=for-the-badge&color=58a6ff)](https://github.com/Utsav-Thakur)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/utsav-thakur)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:utsavthakur.work@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://utsav-thakur-portfolio.vercel.app)
...
<img src="https://skillicons.dev/icons?i=py,cpp,r,js,rust,pytorch,sklearn,postgres,sqlite,fastapi,react,nextjs,tailwind,git,docker,linux&theme=dark" alt="Tech Stack Icons" />
...
current_focus = {
    ...
    "Learning"   : "GNNs, Advanced LLM Finetuning, OCI Deployment & Japanese N5",
}
```
...
- **Credit Risk Engine** — Trained an XGBoost scorecard on 200K LendingClub loans, achieving **0.812 ROC-AUC** and **0.47 KS Stat**. Configured local TreeSHAP models to generate regulator-compliant waterfall feature attributions for individual decisions.
...
- **Forge AI** — Built a local, zero-API RAG assistant for SEC EDGAR 10-K/10-Q filing interrogation using FAISS and sentence-transformers (runs fully offline, no paid API keys required).
...
*SQL-driven analytics and segmentation profiling 3,900 customers across 50 US states to identify promotional trap risks.*
...
- **JARVIS Chatbot** — A local, client-side analytical chatbot (zero API cost) answering 15 customer query categories via pure JS processing over a dynamic DataContext.
- **Business Impact** — Identified **1,677 promo-trap customers (43% of the base)** at immediate margin risk, backed by a 3-phase strategic promo sunset plan.
...
- **Churn Prediction** — Engineered an XGBoost model with 16 features, strictly preventing data leakage via a June 2018 temporal cutoff. Achieved **0.941 ROC-AUC**, **0.982 Precision**, and **0.799 F1-Score**.
- **CLV Interrogation** — Proved standard CLV over/underestimates value for 50.7% of members (4,228 overestimated, 4,147 underestimated) using forward-scoring composite metrics.
- **KMeans Segmentation** — Categorized members into three personas: High-Value Loyalists (4.2% churn), Dormant Members (100% churn, $8.6K avg CLV), and At-Risk Regulars (30.5% churn).
- **Claude AI Retention Engine** — Integrated FastAPI Server-Sent Events (SSE) to stream personalized, segment-specific email campaigns generated by Claude.
...
*Logistics network analysis resolving bottleneck delay models across 144,867 trip segments and 1,508 facilities.*

- **Directed Weighted Graph** — Built a NetworkX graph with 1,508 nodes and 2,847 edges, weighting edges dynamically by median delay ratios, corridor route types, and time of day.
...
- **Bottleneck Hub Analysis** — Used Betweenness Centrality to locate the top 5 bottleneck hubs responsible for **41%** of all network delays.
- **Financial Impact** — Quantified **₹63.8Cr** in annual savings potential via a 6-page operations strategy memo, with interactive Leaflet/Folium heatmaps.
...
- **Hybrid Blend** — Ensembled all three models to achieve top performance: **Precision@10: 0.47, NDCG@10: 0.44**.
- **Datasets** — Scaled across IMDb, TMDb (45K poster URLs via CDN), MovieLens (25M), and MyAnimeList (2023).
- **Cold-Start Mitigation** — Automatic switching to high-weight content recommendations (0.80 weight) for users with fewer than 5 ratings.
...
- **LendIQ** — Generated a correlated 5,000-row synthetic lending dataset, built an XGBoost credit scorecard and Early Warning System dashboard, and delivered a comprehensive Credit Policy Report.
- **Quantum-Inspired Grey Wolf Optimizer (QIGWO)** — Optimized a 10-city Traveling Salesman Problem in Python, yielding a **20% cost reduction** vs. random search baselines, with an interactive Leaflet mapping page.
